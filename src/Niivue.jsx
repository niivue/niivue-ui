...
export default function NiiVue(props) {
  ...
  const [clipPlane, setClipPlane] = React.useState(nv.currentClipPlaneIndex > 0)
  ...
  function nvUpdateCrosshairOpacity(a) {
    nv.setCrosshairColor([crosshairColor[0], crosshairColor[1], crosshairColor[2], a])
    setCrosshairOpacity(a)
  }

  function nvUpdateClipPlaneOpacity(a) {
    nv.opts.clipPlaneColor = [clipPlaneColor[0], clipPlaneColor[1], clipPlaneColor[2], a]
  ...
  return (
    ...
    >
      <SettingsPanel open={openSettings} width={300} toggleMenu={toggleSettings}>
        <ColorPicker colorRGB01={backColor} onSetColor={nvUpdateBackColor} title={'Background color'} />
        <ColorPicker colorRGB01={clipPlaneColor} onSetColor={nvUpdateClipPlaneColor} title={'Clip plane color'} />
        <NumberPicker
          ...
          step={0.1}
        />
        <ColorPicker colorRGB01={crosshairColor} onSetColor={nvUpdateCrosshairColor} title={'Crosshair color'} />
        <NumberPicker
        ...
        <NumberPicker
          ...
          step={1}
        />
        <NumberPicker value={textSize} onChange={nvUpdateTextSize} title={'Text size'} min={0} max={0.2} step={0.01} />
        <ColorPicker colorRGB01={rulerColor} onSetColor={nvUpdateRulerColor} title={'Ruler color'} />
        <NumberPicker
          value={rulerWidth}
        ...
        <NumberPicker
          ...
          step={0.1}
        />
        <NVSwitch checked={locationTableVisible} title={'Location table'} onChange={toggleLocationTable} />
        <NVSwitch checked={orientCube} title={'Orientation cube'} onChange={nvUpdateOrientCube} />
        <NVSwitch checked={ruler} title={'Ruler'} onChange={nvUpdateRuler} />
        <NVSwitch checked={clipPlane} title={'Clip plane'} onChange={nvUpdateClipPlane} />
        <NVSwitch checked={cornerText} title={'Corner text'} onChange={nvUpdateCornerText} />
        <NVSwitch checked={radiological} title={'radiological'} onChange={nvUpdateRadiological} />
        <NVSwitch checked={crosshair3D} title={'3D crosshair'} onChange={nvUpdateCrosshair3D} />
        <NVSwitch checked={colorBar} title={'Show color bar'} onChange={nvUpdateColorBar} />
        <NVSwitch checked={worldSpace} title={'World space'} onChange={nvUpdateWorldSpace} />
        <NVSwitch checked={sagittalNoseLeft} title={'Nose left'} onChange={nvUpdateSagittalNoseLeft} />
        <NVSwitch checked={dragToMeasure} title={'Drag to measure'} onChange={nvUpdateDragToMeasure} />
        <NVSwitch checked={highDPI} title={'High DPI'} onChange={nvUpdateHighDPI} />
        <NumberPicker
      ...
      <LayersPanel open={openLayers} width={320} onToggleMenu={toggleLayers} onAddLayer={addLayer}>
        {layerList}
      </LayersPanel>
      <Toolbar nvUpdateSliceType={nvUpdateSliceType} toggleSettings={toggleSettings} toggleLayers={toggleLayers} />
      <NiivuePanel nv={nv} volumes={layers} />
      <LocationTable tableData={locationData} isVisible={locationTableVisible} decimalPrecision={decimalPrecision} />
    </Box>
  )
}
